...
export function Entropy({ className = "", size = 400, quote }: EntropyProps) {
    ...
    class Particle {
      ...
      update() {
        if (this.order) {
          ...
          // 混合有序运动和混沌影响 - slowed down
          this.x += dx * 0.02 * (1 - this.influence) + chaosInfluence.x * this.influence * 0.4
          this.y += dy * 0.02 * (1 - this.influence) + chaosInfluence.y * this.influence * 0.4

          // 影响逐渐减弱
          this.influence *= 0.995
        } else {
          // 混沌运动 - slowed down
